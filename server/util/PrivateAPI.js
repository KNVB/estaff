...
export default function PrivateAPI(jwt, systemParam) {
    ...
    router.use(async (req, res, next) => {
        ...
        try {
            req.jwt = jwt;
            req.usrObj = await jwt.isValid(token);
            next();
        } catch (error) {
    ...
    router.get('/:action', async (req, res, next) => {
        switch (req.params.action) {
            case "getActiveShiftList":
                sendResponse(res, getActiveShiftList, null, req.jwt, req.usrObj);
                break
            case "getNonStandardWorkingHourList":
                sendResponse(res, getNonStandardWorkingHourList, { month: req.query.month, "year": req.query.year }, req.jwt, req.usrObj);
                break;
            case "getStaffList":
                sendResponse(res, getStaffList, null, req.jwt, req.usrObj);
                break;
            case "getRosterSchedulerData":
                sendResponse(res, getRosterSchedulerData, { month: req.query.month, "systemParam": systemParam, "year": req.query.year }, req.jwt, req.usrObj);
                break;
            default:
                next();
                break;
    ...
    router.post('/:action', async (req, res, next) => {
        switch (req.params.action) {
            case "addNonStandardWorkingHourRecord":
                sendResponse(res, addNonStandardWorkingHourRecord, req.body.record, req.jwt, req.usrObj);
                break;
            case "addStaffInfo":
                console.log(req.body.staffInfo);
                sendResponse(res, addStaffInfo, req.body.staffInfo, req.jwt, req.usrObj);
                break;
            case "deleteNonStandardWorkingHourRecord":
                sendResponse(res, deleteNonStandardWorkingHourRecord, req.body.recordId, req.jwt, req.usrObj);
                break;
            case "updateNonStandardWorkingHourRecord":
                sendResponse(res, updateNonStandardWorkingHourRecord, req.body.record, req.jwt, req.usrObj);
                break
            case "updateStaffInfo":
                sendResponse(res, updateStaffInfo, req.body.staffInfo, req.jwt, req.usrObj);
                break;
            case "updateRoster":
                sendResponse(res, updateRoster, req.body, req.jwt, req.usrObj);
                break;
...
//====================================================================================================================================
let sendResponse = async (res, action, param, jwt, usrObj) => {
    try {
        //res.send(await action(param));
        delete usrObj.iat;
        delete usrObj.exp;       
        let returnObj={
            "accessToken": jwt.sign(usrObj),
